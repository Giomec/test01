...
rate %>% 
  ...
  ggballomnplot(aes(x = Plazo_Ini, y = Tasa_Mid, color=Precio_Mid )) +
...
ggsave(filename = "./test Análisis Tasas Banco Falabella.png", 
       plot = last_plot(),
...
p<-rate %>% 
  filter(Instrumento=="BANCARIA CLF" | Instrumento=="BANCARIA CLP") %>% 
...
anim_save(filename = "./test Animation Tasas Banco Falabella.gif")
getwd()
